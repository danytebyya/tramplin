...
def test_list_public_opportunities_returns_seeded_it_feed(client, db_session):
    ...
    assert all("city" in item for item in items)
    assert all("address" in item for item in items)
    assert all("published_at" in item for item in items)
    assert all("event_type" in item for item in items)
    assert all("mentorship_direction" in item for item in items)
    assert all("mentor_experience" in item for item in items)
